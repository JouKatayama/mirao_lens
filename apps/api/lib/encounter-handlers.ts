import {
  authenticateEncounterHistorySession,
  EncounterHistoryRepositoryError,
} from "@miraio/db";
import { scanRecordSchema, type EncounterHistoryItem } from "@miraio/domain";
...
type EncounterRepositoryPort = Readonly<{
  getEncountersForScan(scanId: string): Promise<EncounterHistoryItem[]>;
}>;

type EncounterSession = Readonly<{
  repository: EncounterRepositoryPort;
  userId: string;
}>;

export type EncounterHandlerDependencies = Readonly<{
  authenticate(accessToken: string): Promise<EncounterSession | null>;
}>;
...
async function authenticateRequest(
  request: Request,
  dependencies: EncounterHandlerDependencies,
): Promise<EncounterSession | Response> {
...
export function createGetEncountersHandler(
  dependencies: EncounterHandlerDependencies,
  ...
  return async (request, context) => {
    ...
    try {
      const items = await session.repository.getEncountersForScan(scanId);
      return jsonResponse({ items, scan_id: scanId });
    } catch (error) {
      return error instanceof EncounterHistoryRepositoryError
        ? internalError("encounter_read_failed")
        : internalError("encounter_request_failed");
    }
  };
}

export function ENCOUNTERS_OPTIONS(): Response {
  return new Response(null, { headers: corsHeaders, status: 204 });
}

export const productionEncounterHandlerDependencies: EncounterHandlerDependencies =
  {
    async authenticate(accessToken) {
      return authenticateEncounterHistorySession(
